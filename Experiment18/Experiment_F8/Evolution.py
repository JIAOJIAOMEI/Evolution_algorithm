...
def evolution_loop(algorithm_parameters):
    ...
    function_name = 'F8'
    with open("genotype_" + function_name + ".txt", "a") as f:
        f.write(str([individual.genotype for individual in population]) + "\n")
    with open("fitness_" + function_name + ".txt", "a") as f:
        f.write(str([individual.fitness for individual in population]) + "\n")
    with open("phenotype_" + function_name + ".txt", "a") as f:
        f.write(str([individual.phenotype for individual in population]) + "\n")
    with open("best_fitness_" + function_name + ".txt", "a") as f:
    ...
    while num_generations_counter <= num_generations and num_evaluations_counter <= num_evaluations:
        if num_generations_counter % 500 == 0:
            print(colorama.Fore.RED + "The number of generations is: " + str(num_generations_counter))
            with open("genotype_" + function_name + ".txt", "a") as f:
                f.write(str([individual.genotype for individual in population]) + "\n")
            with open("fitness_" + function_name + ".txt", "a") as f:
                f.write(str([individual.fitness for individual in population]) + "\n")
