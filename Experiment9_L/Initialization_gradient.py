...
class Individual:
    def __init__(self, func, num_genes, genotype_range, genotype, pattern, K, L, R):
        if pattern == 0:
        ...
        self.phenotype = gradient_search(genotype=self.genotype, func=func, num_genes=num_genes, K=K, L=L,
                                         genotype_range=genotype_range,R=R)


def gradient_search(genotype, func, num_genes, K, L, genotype_range, R):
    ...
    for i in range(L):  # l iterations
        ...
        best_sample = k_list[best_sample_index].copy()  # find best sample, length of best sample = length of genotype
...
def initialization(func, num_genes, num_individual, genotype_range, K, L,R):
    individuals = []
    for _ in range(0, num_individual):
        individuals.append(Individual(func=func, num_genes=num_genes, genotype=None,
                                      genotype_range=genotype_range, pattern=0, K=K, L=L,R=R))
    return individuals
