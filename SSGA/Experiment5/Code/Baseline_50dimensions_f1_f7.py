...
if __name__ == '__main__':
    ...
    with pd.ExcelWriter("Baseline_table_short_best_combination_f1_f7.xlsx") as writer:
        for i in range(0, len(com), 1):
            print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
            data, short_data, temp = multipleF(Times=10, L=[0, 1], Com=com[i],function_list=[1,2,3,4,5,6,7])
            # with open("Baseline_3Dlist_best_combination_f1_f7.txt", "a") as w:
