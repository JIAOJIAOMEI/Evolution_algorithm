...
def test(function, mode, parameter_list):
...
def multiple(mode, Times, L):
    ...
    com = combination()
    k = ["F" + str(i) for i in range(12, 13, 1)]
    for i in range(L[0], L[1], 1):
        result_list_coloum = []
        result_list_coloum_four = []
        for f in range(12, 13, 1):
            result_list = []
    ...
    data = pd.DataFrame(data=np.resize(temp, (L[1] - L[0], len(k) * Times)).swapaxes(0, 1),
                        index=pd.MultiIndex.from_product(
                            [k, ["Times" + str(times + 1) for times in range(Times)]]),
                        columns=[str(mode) + str(i + 1) for i in range(L[0], L[1], 1)]
                        )
    data_four = pd.DataFrame(data=np.resize(temp_four, (L[1] - L[0], len(k) * 4)).swapaxes(0, 1),
                             index=pd.MultiIndex.from_product(
                                 [k, ['Mean', 'Std', 'Max', 'Min']]),
                             columns=[str(mode) + str(i + 1) for i in range(L[0], L[1], 1)]
                             )
...
if __name__ == '__main__':
    ...
    with open("Baldwin_3Dlist_short_1_3072_f12.txt", "w") as w:
        w.write(np.array2string(temp, formatter={'float_kind': lambda x: '{:1.2e}'.format(x)}))
    with open("Baldwin_3Dlist_short_four_1_3072_f12.txt", "w") as w:
        w.write(np.array2string(temp_four, formatter={'float_kind': lambda x: '{:1.2e}'.format(x)}))

    with open("Baldwin_3Dlist_long_1_3072_f12.txt", "w") as w:
        w.write(np.array2string(temp))
    with open("Baldwin_3Dlist_long_four_1_3072_f12.txt", "w") as w:
        w.write(np.array2string(temp_four))

    short_data.to_csv('./Baldwin_table_short_1_3072_f12.csv',
                      sep=',',
                      header=True,
                      index=True)
    short_data_four.to_csv('./Baldwin_table_short_four_1_3072_f12.csv',
                           sep=',',
                           header=True,
                           index=True)

    data.to_csv('./Baldwin_table_long_1_3072_f12.csv',
                sep=',',
                header=True,
                index=True)
    data_four.to_csv('./Baldwin_table_long_four_1_3072_f12.csv',
                     sep=',',
                     header=True,
                     index=True)
     ...
     with open("Lamarck_3Dlist_short_1_3072_f12.txt", "w") as w:
         w.write(np.array2string(temp, formatter={'float_kind': lambda x: '{:1.2e}'.format(x)}))
     with open("Lamarck_3Dlist_short_four_1_3072_f12.txt", "w") as w:
         w.write(np.array2string(temp_four, formatter={'float_kind': lambda x: '{:1.2e}'.format(x)}))
    
     with open("Lamarck_3Dlist_long_1_3072_f12.txt", "w") as w:
         w.write(np.array2string(temp))
     with open("Lamarck_3Dlist_long_four_1_3072_f12.txt", "w") as w:
         w.write(np.array2string(temp_four))
    
     short_data.to_csv('./Lamarck_table_short_1_3072_f12.csv',
                       sep=',',
                       header=True,
                       index=True)
     short_data_four.to_csv('./Lamarck_table_short_four_1_3072_f12.csv',
                            sep=',',
                            header=True,
                            index=True)
    
     data.to_csv('./Lamarck_table_long_1_3072_f12.csv',
                 sep=',',
                 header=True,
                 index=True)
     data_four.to_csv('./Lamarck_table_long_four_1_3072_f12.csv',
                      sep=',',
                      header=True,
                      index=True)
