...
from Initialization_normal import initialization, Individual
...
def test(function, mode, parameter_list, opt):
    ...
    individuals = initialization(func=func, num_genes=num_genes,
                                 num_individual=num_individuals, genotype_range=genotype_range, local_search_rate=local_search_rate, local_search_type=local_search_type, R=R)
    best_generation = []
    ...
    new = Individual(func=func, num_genes=num_genes, genotype=new_individual,
                     genotype_range=genotype_range, pattern=1, local_search_rate=local_search_rate, local_search_type=local_search_type, R=R)
    ...
    for generation in range(iterations - 1):
        ...
        if generation % 2000 == 1998:
            print(
                Fore.BLUE + f'Iter at {generation},  best solution is {best_fv:.5E},  optima is {opt},  similarity is {similarity_population:.5E},   phenotype is {best_individual.genotype}')
        current_optimal = precision_f(best_fv=best_fv, f=function)
        if current_optimal <= opt:
            print(Fore.RED + f"current optimal is {current_optimal},global minima is {opt},similarity is {similarity_population}")
            break
    ...
    for i in range(L[0], L[1], 1):
        ...
        for f in function_list:
            ...
            for times in range(Times):
                print(Fore.YELLOW + f"Times is {times}")
...
def precision_f(best_fv, f):
    f = "F" + str(f)
    if f in ["F1", "F2", "F3", "F4", "F5", "F6", "F7","F8", "F9", "F10", "F11", "F12", "F13", "F14", "F18"]:
