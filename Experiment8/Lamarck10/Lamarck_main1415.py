...
if __name__ == '__main__':
    ...
    com_df = pd.read_csv("./best_20_pm.csv",
                         index_col=[0],
                         header=0)
    com_df["num_phenotypes"] = 10
    print(com_df)
    com = com_df.values.tolist()[14:16]
    index = com_df.index.tolist()[14:16]
    
    function_list = [i for i in range(1, 24, 1)]

    with pd.ExcelWriter("Lamarck1415.xlsx") as writer:
