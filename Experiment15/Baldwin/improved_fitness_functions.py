...
def F10(x):
    ...
    fitness = (-20) * exp(-0.2 * sqrt(reverse * part1)) + (-exp(reverse * part2) + 20 + e)
    return fitness
...
#
# x = [1 for i in range(400)]
# F_list = [F5,F13]
# global_minima = [F(x) for F in F_list]
# print(f"dimension is 50")
# print([str(i)+":"+ str(k) for i,k in zip(F_list,global_minima)])
#
# x = [-0.5 for i in range(400)]
# F_list = [F6]
# global_minima = [F(x) for F in F_list]
# print(f"dimension is 50")
# print([str(i)+":"+ str(k) for i,k in zip(F_list,global_minima)])
